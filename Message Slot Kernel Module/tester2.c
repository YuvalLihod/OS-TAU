...
int perform_random_write(int fd) {
    ...
    if (rc == -1) {
        if(channel>0) {
            fprintf(stderr, "(1)perform_random_write: ioctl failed with error: %d\n", errno);
    ...
    if(length>0 && length <=BUFF_SIZE) {
        if (rc == -1) {
            passed = 0;
            fprintf(stderr, "(2)perform_random_write: write failed with error: %d\n", errno);
            fprintf(stderr, "DETAILS:\n"
                            "fd is - %d,\n"
                            "bffr to write is - %s,\n"
                            "bffr length is - %d.\n", fd, bffr, length);
...
int perform_random_read(int fd){
    ...
    if (rc == -1) {
        if(channel>0) {
            fprintf(stderr, "(1)perform_random_read: ioctl failed with error: %d\n", errno);
            fprintf(stderr, "DETAILS:\n"
                            "fd is - %d,\n"
                            "channel ID is - %d.\n", fd, channel);
        }
    ...
    if (rc == -1) {
        if(channels_msg[channel]==0){
            ...
            passed = 0;
            fprintf(stderr, "(2)perform_random_read: read failed with error: %d\n", errno);
            fprintf(stderr, "DETAILS:\n"
                            "fd is - %d,\n"
                            "channel ID is - %d,\n"
